...
class PatienClarking extends React.Component {
  ...
  handleSubmit = (event) => {
     ...
     * section.
     */
    let presenting_complaints = JSON.parse(
      localStorage.getItem('presenting_complaints')
    );
    let history = JSON.parse(localStorage.getItem('history'));
    let system_examination = JSON.parse(
      localStorage.getItem('system_examination')
    );
    let vital_signs = JSON.parse(localStorage.getItem('vital_signs'));
    let problems = JSON.parse(localStorage.getItem('problems'));
    let lab_investigation = JSON.parse(
      localStorage.getItem('lab_investigation')
    );
    let provisional_diagnosis = JSON.parse(
      localStorage.getItem('provisional_diagnosis')
    );
    let athropometry = JSON.parse(localStorage.getItem('athropometry'));
    let management_plan = localStorage.getItem('management_plan');
    let prescription_request = JSON.parse(
      localStorage.getItem('prescription_request')
    );
    ...
    let previous_medical_history = JSON.parse(
      localStorage.getItem('previous_medical_history')
    );
  ...
  componentWillUpdate(nextProps, nextState) {
    localStorage.setItem(
      'patientrecords',
      JSON.stringify(nextState.patientrecords)
    );
  }
  ...
  getFullDiagnosisByTransactionId = (transactionId) => {
    ...
    const fullDiagnosisByTransactionId = fullDiagnosis.filter(
      (diag) => diag.transactionId === transactionId
    )[0];
